...
JotForm = {
    ...
    handleScreenshot: function(){
        $$('.form-screen-button').each(function(button){
            ...
            button.observe('click', function(){
                JotForm.takeScreenShot(button.id.replace('input_', ''));
            });
            
            /*
            if(window.parent && window.parent.JotFormTakeScreenshot){
                JotForm.getContainer(button).show();
                button.observe('click', function(){
                    window.parent.JotFormTakeScreenshot.getScreen(function(data){
                        $(button.id.replace('input_', 'data_')).value = data;
                        $(button.id.replace('input_', 'screen_')).update('<a href="'+data+'" target="_blank"><img width="100%" src="'+data+'" /></a>');
                    });
                });
            }
            */
    ...
    takeScreenShot: function(id){
        var p = window.parent;
        var uniq = JotForm.uniqid();
        var form = new Element('form', {action:JotForm.server, target:'screen_frame', id:'screen_form', method:'post'});
        ...
        p.$$('.protoplus-dim, .jotform-feedback-link').invoke('hide');
        p.$A(p.document.openWindows).invoke('hide');
        var parentSource = p.document.getElementsByTagName('html')[0].innerHTML;
        
        var html      = new Element('textarea', {name:'html'});
        html.value    = doc+parentSource+"</html>";
        var height    = new Element('input', {type:'hidden', name:'height'}).putValue(parseFloat(p.document.body.clientHeight));
        var scrollTop = new Element('input', {type:'hidden', name:'scrollTop'}).putValue(p.scrollY);
        var uid       = new Element('input', {type:'hidden', name:'uniqID'}).putValue(uniq);
        var action    = new Element('input', {type:'hidden', name:'action'}).putValue("getScreenshot");
        var iframe    = new Element('iframe', {name:'screen_frame', id:'screen_frame_id'});
        iframe.observe('load', function(){
            ...
            var a = new Ajax.Jsonp(JotForm.url + 'server.php', {
                ...
                onComplete : function(t) {
                    ...
                    p.$(p.document.body).insert(res.template);
                    p.$('jt-screen-paper').update('<img width="100%" src="'+JotForm.url+'shots/'+uniq+'.jpg" />');
                    function showback(){
                        p.$$('.protoplus-dim, .jotform-feedback-link').invoke('show');
                        p.$A(p.document.openWindows).invoke('show');
                    }
                }
            });
            ...
            $("screen_"+id).update('<a href="'+JotForm.url+'shots/'+uniq+'.jpg" target="_blank"><img width="100%" src="'+JotForm.url+'shots/'+uniq+'.jpg" /></a>');
        });
        form.insert(html).insert(height).insert(scrollTop).insert(action).insert(uid);
                ...
                try {
                    ...
                    upload.validateInput = function(){
                        if (exVal() !== false) { // Make sure other validation completed
                            ...
                            file.fileSize = file.fileSize ? file.fileSize : file.size;
